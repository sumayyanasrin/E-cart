            ...
            <tbody>
              {
               cart?.map((product,index)=>(
                ...
                <td><input type="text" value={product?.quantity} style={{width:"30px"}} readOnly/></td>
                <td className='text-danger'>${product?.totalPrice}</td>
                <td className='btn'><button onClick={()=>dispatch(removeFromCart(product?.id))}><i class="fa-solid fa-trash text-danger"></i></button></td>
              </tr>
               )) 
               }
            </tbody>
